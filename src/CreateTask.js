...
function CreateTask({selectedBackground}) {
  const [taskName, setTaskName] = useState('');
  const [storyPoint, setStoryPoint] = useState('');
  const [assignee, setAssignee] = useState('');
  const [priority, setPriority] = useState('Low');
  const [tags, setTags] = useState([]); // Updated to handle multiple selections
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        alert('Failed to save the task. Please check the console for errors.');
    }
  };
