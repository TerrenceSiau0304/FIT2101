...
const SprintBacklog = ({selectedBackground}) => {
  ...
  const checkAndUpdateSprintStatus = (sprints) => {
    ...
    const sprintUpdates = []; // Array to hold updates
    ...
    const updatedSprints = sprints.map(sprint => {
      ...
      if (startDateString === todayString && sprint.sprintStatus === 'Not started') {
        ...
        sprintUpdates.push({ id: sprint._id, status: 'In progress' });
      } 
      
      if (endDateString === todayString && sprint.sprintStatus !== 'Completed') {
        updatedSprint.sprintStatus = 'Completed';
        sprintUpdates.push({ id: sprint._id, status: 'Completed' });
      }

      return updatedSprint;
    });

    // Now update the statuses outside of the map function
    sprintUpdates.forEach(({ id, status }) => updateSprintStatus(id, status));
  ...
  return (
    <div className="containersprintbacklog">
      ...
      <div className="menu">
        ...
        <button className="menu-item" onClick={() => navigate('/sprint-backlog')}>
          Sprint Board
        </button>
      ...
      <div className="main-content">
        <div className="header">
          <h2>Sprint Board:</h2>
          <button className="create-sprint" onClick={() => navigate('/create-sprint')}>
            Create Sprint
          </button>
        </div>
        ...
        <div className="sprint-box-container">
          {sprints.map((sprint, index) => (
            <div className="sprint-item" key={index}>
              {/* Sprint Details Button */}
              <button className="sprint-box" onClick={() => handleSprintClick(sprint)}>
              ...
              <button 
                ...
                onClick={() => navigate(`/BurnDownChart/${sprint._id}`)} // Pass an arrow function
              >
                Show Graph
              </button>
            </div>
